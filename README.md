...
- command search runs on the language-specific payload returned by the backend. Catalog descriptions are covered for all supported site languages; executable command syntax, aliases, usage, examples, and placeholders stay sourced from plugin metadata so the UI does not invent commands the server cannot execute.
- generated translation coverage is tracked in `CelemBackend/docs/command-translation-coverage.md`; generated descriptions should still be reviewed by native speakers over time.
